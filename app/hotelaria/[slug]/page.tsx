...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }): Promise<Metadata> {
  ...
  return {
    title: `${listing.title} — Acalanto Tours`,
    description: listing.description ?? `Hospedagem em Paraty: ${listing.title}`,
  }
}

export default async function HotelariaSlugPage({ params }: { params: Promise<{ slug: string }> }) {
  const { slug } = await params
  const listing = await getListingBySlug(slug)
  if (!listing) notFound()
  ...
  return (
    <main style={{ paddingTop: '5rem', minHeight: '80vh' }}>
      ...
      <div style={{ height: '320px', background: 'linear-gradient(135deg, var(--ocean-mid), var(--sunset))', position: 'relative', overflow: 'hidden' }}>
        ...
        <div style={{ position: 'absolute', bottom: '2rem', left: '50%', transform: 'translateX(-50%)', width: '100%', maxWidth: '760px', padding: '0 1.5rem' }}>
      ...
      <section style={{ padding: 'clamp(2.5rem, 6vw, 4rem) 1.5rem', background: 'var(--sand)' }}>
        <div className="container" style={{ maxWidth: '760px', display: 'grid', gridTemplateColumns: '1fr minmax(0, 300px)', gap: '2.5rem', alignItems: 'start' }}>
          ...
          <div>
            ...
            {maxGuests && (
              <p style={{ fontSize: '0.875rem', color: 'var(--text-muted)' }}>
                Capacidade: até <strong>{maxGuests} hóspedes</strong>
              </p>
            )}
          ...
          <div style={{ background: 'white', borderRadius: '16px', padding: '1.75rem', border: '1px solid var(--border)', position: 'sticky', top: '6rem' }}>
            ...
            {whatsapp ? (
              <a
                href={`https://wa.me/${whatsapp}?text=Olá! Vi sua hospedagem no site da Acalanto Tours e gostaria de saber a disponibilidade.`}
                target="_blank"
                rel="noreferrer"
              ...
              <a
                href={`https://wa.me/${process.env.NEXT_PUBLIC_WHATSAPP_NUMBER || '5524999627968'}?text=Olá! Tenho interesse na hospedagem "${listing.title}" que vi no site da Acalanto Tours.`}
                target="_blank"
                rel="noreferrer"
                className="btn-primary"
                style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '0.5rem', padding: '1rem', textDecoration: 'none', width: '100%' }}
            ...
            <p style={{ fontSize: '0.75rem', color: 'var(--text-muted)', textAlign: 'center', marginTop: '0.875rem', lineHeight: 1.5 }}>
              Parceiro verificado pela Acalanto Tours
            </p>
          </div>
        </div>
      </section>
